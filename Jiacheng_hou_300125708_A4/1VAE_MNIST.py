...
def main():
	...
	# Model +  Loss function
	# z_dim = 128!
	model = VAE(x_dim=784, h_dim1= 512, h_dim2=256, z_dim=128).to(device)
	...
	plt.savefig(f"1VAE_MNIST_final_loss_epoch.png")
	plt.close()
	...
	plt.savefig(f"1VAE_MNIST_BCE_loss_epoch.png")
	plt.close()
	...
	plt.savefig(f"1VAE_MNIST_KLD_loss_epoch.png")
	plt.close()
	...
	with torch.no_grad():
		for batch_idx, (images, _ ) in enumerate(test_loader):
			...
			if batch_idx == int(len(test_loader.dataset)/batch_size) - 1:
					...
					save_generator_image(generated_img, f"./1IMAGE_VAE_MNIST/epoch{epoch+1}.png")
