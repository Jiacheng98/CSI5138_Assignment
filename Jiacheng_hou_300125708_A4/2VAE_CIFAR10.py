...
class VAE(nn.Module):
    ...
    def decoder(self, z):
        ...
        # reconstruction = torch.sigmoid(self.conv8(conv7))
        # self.conv8(conv7).view(-1, 3, 32, 32)
...
def main():
	...
	for epoch in range(EPOCHS):
			...
			test_epoch_loss_MSE, test_epoch_loss_KLD, test_epoch_loss = evaluate(model, optimizer, test_loader, BATCH_SIZE, epoch, criterion)
	...
	plt.savefig(f"2VAE_CIFAR10_final_loss_epoch.png")
	plt.close()
	...
	plt.savefig(f"2VAE_CIFAR10_MSE_loss_epoch.png")
	plt.close()
	...
	plt.savefig(f"2VAE_CIFAR10_KLD_loss_epoch.png")
	plt.close()
...
def evaluate(model, optimizer, test_loader, batch_size, epoch, criterion):
	model.eval()
	running_loss = 0.0
	running_loss_KLD = 0.0
	running_loss_MSE = 0.0
	with torch.no_grad():
		for batch_idx, (images, _ ) in enumerate(test_loader):
			...
			if batch_idx == int(len(test_loader.dataset)/batch_size) - 1:
					...
					save_generator_image(generated_img, f"./2IMAGE_VAE_CIFAR10/epoch{epoch+1}.png")
