...
def main():
	...
	# Model +  Loss function
	# z_dim = 512!
	model = VAE(x_dim=3*32*32, h_dim1= 512, h_dim2=256, z_dim=512).to(device)
	...
	for epoch in range(EPOCHS):
		...
		train_epoch_loss_MSE, train_epoch_loss_KLD, train_epoch_loss = train(model, optimizer, train_loader)
		test_epoch_loss_MSE, test_epoch_loss_KLD, test_epoch_loss = evaluate(model, optimizer, test_loader, BATCH_SIZE, epoch)
	...
	plt.savefig(f"2VAE_CIFAR10_final_loss_epoch_mlp.png")
	plt.close()
	...
	plt.savefig(f"2VAE_CIFAR10_MSE_loss_epoch_mlp.png")
	plt.close()
	...
	plt.savefig(f"2VAE_CIFAR10_KLD_loss_epoch_mlp.png")
	plt.close()




def loss_function(mse_loss, recon_x, x, mu, log_var):
	MSE = mse_loss
...
def train(model, optimizer, train_loader):
	...
	for batch_idx, (images, _ ) in tqdm(enumerate(train_loader), total = int(len(train_loader.dataset) / BATCH_SIZE)):
		...
		MSE_loss, KLD_loss, final_loss = loss_function(mse_loss, recon_batch, images, mu, log_var)
		final_loss.backward()
	...
	with torch.no_grad():
		for batch_idx, (images, _ ) in enumerate(test_loader):
			...
			if batch_idx == int(len(test_loader.dataset)/batch_size) - 1:
					...
					save_generator_image(generated_img, f"./2IMAGE_VAE_CIFAR10_mlp/epoch{epoch+1}.png")

					# if epoch == EPOCHS-1:
					# 	image_batch = images.view(batch_size, 1, 28, 28)[:64]
					# 	real_img = make_grid(image_batch, padding =2, normalize = True)
					# 	save_generator_image(real_img, f"./IMAGE_CIFAR10/epoch{epoch+1}.png")
